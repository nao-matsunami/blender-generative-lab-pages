"""Generate Aortic Root Clamp assets in Blender.
...
def clamp_field(u: float, v: float) -> float:
    theta = u * math.tau
    front = smoothstep(0.05, 0.9, 0.5 + 0.5 * math.sin(theta - 0.15))
    band = math.exp(-((v - 0.48) / 0.22) ** 2)
    return front * band


def fold_field(u: float, v: float) -> float:
    theta = u * math.tau
    clamp = clamp_field(u, v)
    rib = 0.0
    for index in range(12):
        center = (index / 12.0 + 0.2 * v + 0.035 * math.sin(v * math.tau)) % 1.0
        distance = abs(((u - center + 0.5) % 1.0) - 0.5)
        rib += max(0.0, 1.0 - distance / 0.032) ** 2.2
    broad = 0.5 + 0.5 * math.sin(theta * 2.0 + v * math.tau * 1.1)
    return min(1.0, clamp * 0.55 + rib * 0.28 + broad * 0.12)
...
def surface_point(u: float, v: float) -> Vector:
    ...
    clamp = clamp_field(u, v)
    fold = fold_field(u, v)
    chamber = 1.0 + 0.13 * math.sin(theta + 0.3) + 0.08 * math.cos(phi * 1.4)
    rx = 50.0 * chamber * (1.0 - 0.3 * clamp) + fold * 5.5
    ry = 36.0 * (1.0 + 0.1 * math.cos(theta * 2.0)) - clamp * 17.0
    rz = 54.0 * (1.0 + 0.08 * math.sin(theta - 0.2))
    x = math.cos(theta) * dome * rx + 4.0 * math.sin(phi * 1.8)
    y = math.sin(theta) * dome * ry - clamp * 14.0
    z = math.sin(phi) * rz + dome * math.sin(theta * 2.0 - 0.8) * 5.5
    return Vector((x, y, z))
...
def color_at(u: float, v: float) -> tuple[float, float, float, float]:
    fold = fold_field(u, v)
    clamp = clamp_field(u, v)
    wet = 0.5 + 0.5 * math.sin(u * math.tau * 2.7 + v * math.tau * 2.1)
    base = (0.68 + wet * 0.14, 0.12 + wet * 0.05, 0.18 + wet * 0.05)
    dark = (0.07, 0.001, 0.015)
    crease = min(0.9, fold * 0.7 + clamp * 0.22)
...
def create_chamber_mesh(material: bpy.types.Material) -> bpy.types.Object:
    ...
    mesh = bpy.data.meshes.new("aortic_root_chamber_mesh")
    mesh.from_pydata(vertices, [], faces)
    mesh.update()
    color_attribute = mesh.color_attributes.new(name="aortic_skin_color", type="BYTE_COLOR", domain="CORNER")
    for polygon in mesh.polygons:
        for loop_index in polygon.loop_indices:
            color_attribute.data[loop_index].color = colors[mesh.loops[loop_index].vertex_index]
    obj = bpy.data.objects.new("Aortic Root Clamp Chamber", mesh)
    bpy.context.collection.objects.link(obj)
    obj.data.materials.append(material)
    bpy.context.view_layer.objects.active = obj
    obj.select_set(True)
    bpy.ops.object.shade_smooth()
    weighted = obj.modifiers.new("aortic_weighted_normals", "WEIGHTED_NORMAL")
...
def material_from_attribute() -> bpy.types.Material:
    material = bpy.data.materials.new("wet_aortic_chamber")
    material.use_nodes = True
    bsdf = material.node_tree.nodes.get("Principled BSDF")
    if bsdf:
        attr = material.node_tree.nodes.new("ShaderNodeAttribute")
        attr.attribute_name = "aortic_skin_color"
...
def add_root_parts(dark_material: bpy.types.Material, tube_material: bpy.types.Material, pressure_material: bpy.types.Material) -> list[bpy.types.Object]:
    objects: list[bpy.types.Object] = []
    bpy.ops.mesh.primitive_uv_sphere_add(segments=80, ring_count=30, radius=1, location=(0, -34, 4))
    void = bpy.context.object
    void.name = "aortic_black_root_socket"
    void.scale = (24, 9, 24)
    ...
    tubes = [
        ("aortic_upper_root", [(-8, -34, 4), (-4, -45, 24), (2, -54, 47), (8, -60, 68)], 7.0),
        ("aortic_left_root", [(-6, -34, 1), (-23, -43, 8), (-38, -48, 13), (-52, -49, 16)], 6.0),
        ("aortic_right_root", [(7, -34, 1), (23, -43, -3), (37, -48, -8), (52, -49, -14)], 5.8),
    ]
    for name, points, bevel in tubes:
        objects.append(make_tube(name, points, bevel, tube_material))

    for index, location in enumerate([(-14, -35, 2), (13, -35, 0), (0, -37, 16)]):
        bpy.ops.mesh.primitive_uv_sphere_add(segments=56, ring_count=18, radius=1, location=location)
        dent = bpy.context.object
        dent.name = f"aortic_pressed_contact_pad_{index + 1}"
        dent.scale = (12, 3.8, 9)
        dent.rotation_euler = (0, 0, math.radians(index * 38 - 20))
        dent.data.materials.append(pressure_material)
        bpy.ops.object.shade_smooth()
        objects.append(dent)
    return objects
...
def setup_scene(objects: list[bpy.types.Object]) -> None:
    ...
    for obj in objects:
        ...
        obj.location.z += 72
    bpy.ops.object.empty_add(type="PLAIN_AXES", location=(0, -18, 76))
    target = bpy.context.object
    target.name = "aortic_camera_target"
    bpy.ops.object.camera_add(location=(-20, -252, 134), rotation=(math.radians(62), 0, math.radians(-5)))
    ...
    bpy.ops.object.light_add(type="AREA", location=(-126, -132, 194))
    key = bpy.context.object
    key.data.energy = 6600
    key.data.color = (1.0, 0.48, 0.45)
    key.data.size = 150
    bpy.ops.object.light_add(type="POINT", location=(128, 72, 136))
    rim = bpy.context.object
    rim.data.energy = 2300
    rim.data.color = (0.8, 0.02, 0.12)
    rim.data.shadow_soft_size = 86
    bpy.ops.object.light_add(type="AREA", location=(0, -194, 104))
...
def add_preview_backdrop() -> None:
    mat = make_material("aortic_preview_dark_backdrop", (0.006, 0.001, 0.003, 1.0), 0.95)
    bpy.ops.mesh.primitive_plane_add(size=1.0, location=(0, 150, 76), rotation=(math.radians(90), 0, 0))
    backdrop = bpy.context.object
    backdrop.name = "aortic_preview_dark_backdrop"
...
def main() -> None:
    ...
    dark_material = make_material("near_black_aortic_socket", (0.005, 0.0, 0.003, 1.0), 0.62)
    tube_material = make_material("thick_dark_crimson_roots", (0.48, 0.025, 0.075, 1.0), 0.17, 0.05)
    pressure_material = make_material("compressed_root_contact_pads", (0.78, 0.15, 0.20, 1.0), 0.18, 0.04)
    objects = [chamber]
    objects.extend(add_root_parts(dark_material, tube_material, pressure_material))
    setup_scene(objects)
    export_stl(objects)
    export_glb(objects)
    render_outputs()
